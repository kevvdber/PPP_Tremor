 function EMGACC_runcluster_25APR2024(Task)
...
addpath(genpath('/project/3024011.01/TremorProject/scripts/kevin/EMG_ACC/'));
addpath(genpath('/project/3024011.01/TremorProject/scripts/kevin/eeglab14_0_0b'));
addpath(genpath('/project/3024011.01/TremorProject/scripts/kevin/Personalized-Parkinson-Project-Physiological-Recordings'));
%% Settings
...
conf.todo.prepemg            = true; %When doing frequency analysis, do you want to prepare emg using: pf_emg_raw2regr_prepemg
...
%Task Settings:
POMVisit = 'POMVisit1'; %For POM third visit change in: 'POMVisit3'; 
% POMVisit = 'POMVisit3'; %For POM first visit change in: 'POMVisit1'; 
cluster_outputdir = fullfile('/project/3024011.01/TremorProject/analyses/kevin/TremorNetwork',POMVisit,'EMG', Task, 'clusteroutput');
processing_dir = fullfile('/project/3024011.01/TremorProject/analyses/kevin/TremorNetwork',POMVisit, 'EMG', Task, 'processing');
...
elseif strcmp(Task,'rest')
    ...
    MB          = 8;
    NSlices     = 64 / MB;     
    pScan = '^sub-.*task-rest_acq-MB8.*_bold.nii.gz$';
elseif or(strcmp(Task,'Task1'),strcmp(Task,'Task2'))
    
end

%Slice time correction - set to 'true' if slice time correction has been
%applied - check fmriprep reports
STC = false;
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% SETTINGS pf_emg_raw2regr.m & pf_emg_farm.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%--------------------------------------------------------------------------
%REGR
conf.dir.rootsave  =   fullfile('/project/3024011.01/TremorProject/analyses/kevin/TremorNetwork',POMVisit,'EMG', Task);
conf.dir.ParkFunc  =   '/project/3024011.01/TremorProject/scripts/kevin/EMG_ACC/Helpers/ParkFunC_EMG';     % Directory containing Park_Func
conf.dir.Farm      =   '/project/3024011.01/TremorProject/scripts/kevin/EMG_ACC/Helpers/FARM_toolbox';          % Directory containing Farm
conf.dir.eeglab    =   '/project/3024011.01/TremorProject/scripts/kevin/eeglab14_0_0b'; % Directory containing EEGLAB
...
conf.dir.preproc   =   fullfile(conf.dir.rootsave, 'processing', 'FARM');                     % Directory containing files used for "prepemg" (usually after FARM)
conf.dir.prepemg   =   fullfile(conf.dir.rootsave, 'processing', 'prepemg');                  % Directory where files from function "prepemg" will be stored
conf.dir.auc       =   fullfile(conf.dir.rootsave, 'auc');                      % Directory for aucs
...
%FARM
conf.dir.save    =   fullfile(conf.dir.rootsave, 'processing', 'FARM');
conf.dir.work    =   fullfile(conf.dir.rootsave, 'processing', 'FARM', 'work');
...
%Start edit kevvdber 29 march 2022 
% include all subjects in specific lists for visit 1, visit 3 or both. 
Sub_POM_visit1 = []; 
Sub_POM_visit3 = []; 
Sub_POM_visitboth = []; 
for i = 1:numel(Sub)
    if exist(fullfile(pBIDSDir,Sub{i,1},'ses-POMVisit1'),'dir') && ~exist(fullfile(pBIDSDir,Sub{i,1},'ses-PITVisit1','dwi'),'dir')
        j = size(Sub_POM_visit1,1)+1;
        Sub_POM_visit1{j,1} = Sub{i,1};
    end 
    if exist(fullfile(pBIDSDir,Sub{i,1},'ses-POMVisit3'),'dir')
        j = size(Sub_POM_visit3,1)+1;
        Sub_POM_visit3{j,1} = Sub{i,1};
    end 
    if (exist(fullfile(pBIDSDir,Sub{i,1},'ses-POMVisit1'),'dir') && exist(fullfile(pBIDSDir,Sub{i,1},'ses-POMVisit3'),'dir'))
        j = size(Sub_POM_visitboth,1)+1;
        Sub_POM_visitboth{j,1} = Sub{i,1};
    end
end 

if strcmp(POMVisit,'POMVisit1')
    Sub = Sub_POM_visit1; Sel = true(numel(Sub),1);
% elseif POMVisit == 'POMVisit3'
%     Sub = Sub_POM_visit3; Sel = true(numel(Sub),1);
elseif strcmp(POMVisit,'POMVisit3') %|| POMVisit == 'POMVisitboth'
    Sub = Sub_POM_visitboth; Sel = true(numel(Sub),1);
end 

%End edit kevvdber 29 march 2022 

%Exclude subjects without task data
for n = 1:numel(Sub)
    Sessions = cellstr(spm_select('FPList', fullfile(pBIDSDir, Sub{n}), 'dir', strcat('ses-',POMVisit)));
    TaskData_fmri = cellstr(spm_select('List', fullfile(Sessions, 'func'), ['.*task-', Task, '_acq-MB8.*bold.nii.gz']));
    TaskData_beh  = cellstr(spm_select('List', fullfile(Sessions, 'beh'), ['.*.json']));
...
for n = 1:numel(Sub)
    Sessions = cellstr(spm_select('FPList', fullfile(pBIDSDir, Sub{n}), 'dir', strcat('ses-',POMVisit)));
    vmrk    = cellstr(spm_select('List', fullfile(Sessions, 'eeg'), ['.*task-', Task, '.*_eeg.vmrk']));
    eeg     = cellstr(spm_select('List', fullfile(Sessions, 'eeg'), ['.*task-', Task, '.*_eeg.eeg']));
    vhdr    = cellstr(spm_select('List', fullfile(Sessions, 'eeg'), ['.*task-', Task, '.*_eeg.vhdr']));
...
Sub = Sub(Sel);
% load('/project/3024006.03/TremorProject/scripts/kevvdber/EMG_ACC/inputtable_visit3.mat');
% Sub = cellstr(inputTable_V3_complete.Subject);
% Sub = Sub(1:51,:); %for testing purposes, select only subset 
...
for n = 1:numel(Sub)
conf.sub.name   = Sub(n);
Sessions = cellstr(spm_select('FPList', fullfile(pBIDSDir, conf.sub.name), 'dir', strcat('ses-',POMVisit)));
%     for i = 1:numel(Sessions)
    i = 1; %only one run for now; see also Settings and line 195
    [~, conf.sub.sess, ~] = fileparts(Sessions{i});
%     conf.sub.sess = cellstr(conf.sub.sess);
    conf.sub.sess = {strcat('ses-',POMVisit)}; %Edited to only perform runs on visits specified in Settings, e.g. POMVisit1 or POMVisit3
...
%     end
    ...
    %
    clear setup_cfg;
    setup_cfg    = ft_read_header(spm_select('FPList', fullfile(pBIDSDir, conf.sub.name{1}, conf.sub.sess{1}, 'eeg'), ['^sub.*' Task '_eeg.eeg$']));
    emgChannels = sort(vertcat(find(contains(setup_cfg.label,'_right')), find(contains(setup_cfg.label,'_left'))))';
    accChannels = sort(find(contains(setup_cfg.label,'accelerometer')))';
    %
...
%     end
    ...
    %
    conf.prepemg.chan = setup_cfg.label;
    %
...
% --- Plotting options --- %
    ...
    conf.prepemg.subplot.chan          = { conf.prepemg.chan(emgChannels);
        [conf.prepemg.chan(accChannels);'acc-vec'];
...
% --- Optional: combine channels if desired --- %
    ...
    conf.prepemg.combichan.chan = {conf.prepemg.chan(accChannels) 'acc-vec';};  % Choose the sets of channels you want to combine (for every row another set of channels, in the left column the channel names (as in conf.prepemg.chan) in the right column the new name)
    conf.prepemg.combichan.meth = 'vector';                                % Choose method of combining ('vector': will make a vector out of a triaxial signal (x^2+y^2+z^2)^0.5 | 'pca': performs principal component analysis and will take first principle component)

% --- Optional: perform coherence analysis if desired --- %
    conf.prepemg.cohchan.on         =  'no';    % Cohere channels (specify 'yes')
...
%% Make Regressor ('mkregressor')
%--------------------------------------------------------------------------
    conf.mkregr.art_remove_reg = true; %OPTIONAL: removes artifacts from scans 804 and 805 (depending on dataset) on regressor level, leaving raw data as is
    conf.mkregr.reanalyze = 'no'; %or 'no'  % Choose if you want to reanalyze previously selected data. If not, then use the GUI: pf_emg_raw2regr_mkregressor_gui
...
%     conf.mkregr.automaticfreqwin = [2.99,13.01];
    conf.mkregr.automaticfreqwin = [2.99,7.01];
    conf.mkregr.automaticdir = fullfile(conf.dir.rootsave, 'automaticdir');
...
%     conf.mkregr.sample    = 735/2; 
%     conf.mkregr.sample    = 367;
    if STC==false
         conf.mkregr.sample    = 1; 
    elseif STC==true 
        conf.mkregr.sample    = ceil(TR*1000/2); 
    end
%     conf.mkregr.sample = 1;
...
%SPEFIC TO DONDERS AND OUR STUDY
    if conf.todo.ACC
        if contains(POMVisit,'POMVisit1') | contains(POMVisit,'POMVisit3')
            conf.mkregr.automaticchans = accChannels - 2;
        elseif contains(POMVisit,'mri01') | contains(POMVisit,'mri02')
            conf.mkregr.automaticchans = accChannels;
        end       
    else
...
%--------------------------------------------------------------------------
    ...
    %for estimation of number of scans (nScans)
    dImg = fullfile(pBIDSDir, conf.sub.name, conf.sub.sess, 'func');
    fpImg = spm_select('FPList', dImg, pScan);
    nScans = niftiinfo(fpImg(end,:)).ImageSize(end); %last run
    %
    conf.file.scanpar       =   [TR;NSlices;nScans];                                 % Scan parameters: TR / nSlices / nScans (enter nan for nScans if you want to automatically detect this)
...
%% Call functions
    ...
    %Check number of scans
        ...
        fpImg = fpImg(end,:);
        nScan = nScans;
%         nScan = niftiinfo(fpImg).ImageSize(end);
%         nScan = size(spm_vol(fpImg(size(fpImg,1),:)),1);    % Takes the last run only
        conf.mkregr.nscan = nScan - conf.prepemg.dumscan;
%         conf.dir.root      =   fullfile('/project/3022026.01/pep/bids',conf.sub.name{1},conf.sub.sess{1},'eeg/'); %% root directory, where all raw EMG files are located
%         conf.dir.root = fullfile('/project/3024006.03/TremorProject/analyses/EMG/Data_adjustedmarkers/');
        conf.dir.root = fullfile('/project/3024011.01/TremorProject/analyses/kevin/TremorNetwork/adjustedmarkers/');
%         conf.dir.root = fullfile(pBIDSDir,Sub{i,1},strcat('ses-',POMVisit),'eeg');
        fprintf(['\n --- Submitting FARM-job for subject ' conf.sub.name{1} ' ---\n']);
        FARMjobs{n} = qsubfeval('pf_emg_farm', conf.sub.name, conf,'memreq', 8*1024^3,'timreq',1*60*60);  % Run on cluster ;
